...
public class QuickSort {
    ...
    public static void quick_sort1(int array[], int left, int right) {
        ...
        int partition = partition(array, left, right);//先成挖坑填数法调整s[]
        quick_sort1(array, left, partition - 1); // 递归调用
    ...
    public static int partition(int[] array, int left, int right) {
        ...
        int i = left, j = right;
        int x = array[left]; //s[l]即s[i]就是第一个坑
        while (i < j) {
            //两个条件，保证i<j
            // 从右向左找小于x的数来填s[i]
            while (i < j && array[j] >= x) {
    ...
    public static int advancedPartition(int arr[], int left, int right) {
        ...
        //arr[left]是一个随机选择数
        //v是一个随机选择数。应付高度有序。
        arr[left] = arr[selectPartitionRandom(arr, left, right)];
        int v = arr[left];
