...
public class PileBox {
  ...
  class Solution {
     ...
     * <p>数据预处理
     *
     * <p>第一点需要明白的是，我们先对box数组按照一个维度进行排序，得到sorted_box序列，
     *
     * <p>那么最终答案的序列就一定是sorted_box序列的某个子序列（这点很重要）。那么之后我们只需要找到这个总高度最大的子序列。
     *
     * <p>设dp[i]表示以第i个箱子为结尾的上升子序列的最大总高度。则可以知道：
     *
     * <p>题目要求在3个维度（宽度 + 深度 + 高度）上整体严格递增。 先排序1个维度（宽度），先保证在1个维度上递增（并非严格递增）。
     *
     * <p>之后专心处理剩余的2个维度（深度 +高度）即可，而这就需要使用动态规划。
     *
     * @param box
     * @return
     */
